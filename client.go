...
// https://api-ms.netangels.ru/api/v1/
import (
	"bytes"
	"context"
	"encoding/json"
...
func init() {
	...
	if apiDebug, ok := os.LookupEnv("NETANGELS_DEBUG"); ok {
		if parsed, err := strconv.ParseBool(apiDebug); err == nil {
			envDebug = parsed
			log.Println("[INFO] NETANGELS_DEBUG being set to", envDebug)
		} else {
			log.Println("[WARN] NETANGELS_DEBUG should be an integer, 0 or 1")
		}
	}
}
...
// SetBaseURL sets the base URL of the Netangels v1 API (https://api-ms.netangels.ru/v1)
...
// from the NETANGELS_CONFIG file and the NETANGELS_TOKEN environment variable.
